...
gentheme() {
	...
	if command -v zip >/dev/null 2>&1; then
		...
		zip -j -FS "$cachedir/$themename" "$tempdir"/* >/dev/null 2>&1 || {
			# create clean zip in case syncing failed
			rm -f "$cachedir/$themename"
			zip -jq "$cachedir/$themename" "$tempdir"/* >/dev/null 2>&1
		}
	else
...
main() {
	...
	[ ! -f "$walfile" ] && [ -z "$wal" ] && error "wallpaper path file not found: $walfile" "run \`wal\` to generate it or pass custom wallpaper path with \`-i\`"
